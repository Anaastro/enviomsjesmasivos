  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {products.map((product) => (
        <div key={product.id} className="bg-black border-4 border-orange-500 p-4 rounded-lg shadow-md">
          {product.images && product.images.length > 0 && product.images.map((image, index) => (
            <img key={index} src={image} alt={product.name} className="w-full h-48 object-cover rounded-lg mb-4" />
          ))}
          <h3 className="text-xl font-bold text-orange-500 mb-2">{product.name}</h3>
